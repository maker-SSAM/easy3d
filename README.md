# 초보자를 위한 파라메트릭 디자인 스튜디오

로그인 없는 웹 파라메트릭 디자인 도구입니다. 이름표, 상자 같은 간단한 디자인을 슬라이더로 조절해서 만들고, 3D 프린팅용 STL 파일로 내려받을 수 있습니다.

> 원래는 초등학생 대상 연구대회 출품용으로 제작을 시작했습니다. 배포를 앞두고 "초등"이라는 한정을 떼어 더 넓은 초보자층(3D 프린팅 입문자 등)을 대상으로 이름을 다듬었지만, UI가 단순해 초등 교실 수업에서도 그대로 활용할 수 있습니다.
